...
==========================================================================*/
static void draw_hand(Region* r, double angle, int cx, int cy, int thickness, int l, BYTE cr, BYTE cg, BYTE cb) {
  int lx = l * sin(angle);
  int ly = l * cos(angle);
  int lxh = l / 10 * sin(angle + M_PI);
  int lyh = l / 10 * cos(angle + M_PI);
  region_draw_hollow_line(r, cx + lxh, cy - lyh, cx + lx, cy - ly, thickness, cr, cg, cb);
}
...
==========================================================================*/
static void draw_date(Region* r, int l, int cx, int cy, BYTE cr, BYTE cg, BYTE cb, const BitmapFont* font) {
  ...
  region_draw_bitmap_text(r, font, s, cx - xo, cy - 2 * text_height, cr, cg, cb);
}
...
==========================================================================*/
static void draw_numerals(Region* r, int l, int cx, int cy, BYTE cr, BYTE cg, BYTE cb, const BitmapFont* font) {
  int text_height = font->height;
  int text_width = font->width;
  for (int i = 0; i < 12; i++) {
    ...
    region_draw_bitmap_text(r, font, s, cx + lx - xo, cy - ly, cr, cg, cb);
  }
}
...
void program_draw_clock_in_region(Region* r, BOOL seconds, BOOL date) {
  ...
  if (date) draw_date(r, lm, cx, cy, cr, cg, cb, font);

  int lm_hands = lm - 2 * font->height;

  if (seconds) draw_hand(r, (double)sec / 60 * TWOPI, cx, cy, 1, lm_hands, cr, cg, cb);                        // sec
  draw_hand(r, (double)min / 60 * TWOPI, cx, cy, 5, lm_hands * 9 / 10, cr, cg, cb);                            // min
  draw_hand(r, ((double)hr / 12 + (double)min / 60 / 12) * TWOPI, cx, cy, 10, lm_hands * 6 / 10, cr, cg, cb);  // hour
}
