...
static inline float ipart(float x) { return floor(x); }

static inline float rnd(float x) { return ipart(x + 0.5); }

static inline float fpart(float x) { return x - floor(x); }
...
/*==========================================================================
  ...
  elsewhere
*==========================================================================*/
void region_set_pixel_t(Region* self, int x, int y, BYTE r, BYTE g, BYTE b, float t) {
  b = (BYTE)(t * (float)b);
  g = (BYTE)(t * (float)g);
  r = (BYTE)(t * (float)r);
  if (x > 0 && x < self->w && y > 0 && y < self->h) {
...
*==========================================================================*/
void region_draw_bitmap_char(Region* self, const BitmapFont* bf, char c, int x, int y, int r, int g, int b) {
  LOG_IN
  if (x < self->w && y < self->h) {
    if (c < ' ' || c > '~') c = '?';
    int char_offset = (c - ' ') * bf->height * (bf->width / 8 + (bf->width % 8 ? 1 : 0));
    const uint8_t* ptr = &bf->table[char_offset];
    for (int page = 0; page < bf->height; page++) {
      for (int column = 0; column < bf->width; column++) {
        if (*ptr & (0x80 >> (column % 8))) region_set_pixel(self, x + column, y + page, r, g, b);
        if (column % 8 == 7) ptr++;
      }
      if (bf->width % 8 != 0) ptr++;
    }
...
/*==========================================================================
  region_get_width
*==========================================================================*/
int region_get_width(const Region* self) { return self->w; }

/*==========================================================================
  region_get_height
*==========================================================================*/
int region_get_height(const Region* self) { return self->h; }
...
*==========================================================================*/
void region_draw_line_one_pixel(Region* self, int x0, int y0, int x1, int y1, BYTE r, BYTE g, BYTE b) {
  LOG_IN
...
*==========================================================================*/
void region_draw_hollow_line(Region* self, int x1, int y1, int x2, int y2, int t, BYTE r, BYTE g, BYTE b) {
  if (t == 1) {
