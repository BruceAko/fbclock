  ...
  if (self->fd >= 0) {
    ...
    for (int y = 0; y < self->h; y++) {
      for (int x = 0; x < self->w; x++) {
        *(self->fb_data + y * self->line_length + x * self->fb_bytes) = 0x1F;
...
*==========================================================================*/
void framebuffer_set_pixel(FrameBuffer* self, int x, int y, BYTE r, BYTE g, BYTE b) {
...
*==========================================================================*/
int framebuffer_get_width(const FrameBuffer* self) { return self->w; }

/*==========================================================================
  framebuffer_get_height
*==========================================================================*/
int framebuffer_get_height(const FrameBuffer* self) { return self->h; }

/*==========================================================================
  framebuffer_get_pixel
*==========================================================================*/
void framebuffer_get_pixel(const FrameBuffer* self, int x, int y, BYTE* r, BYTE* g, BYTE* b) {
  if (x > 0 && x < self->w && y > 0 && y < self->h) {
    int index32 = (y * self->w + x) * self->fb_bytes + (y * self->slop);
...
*==========================================================================*/
BYTE* framebuffer_get_data(FrameBuffer* self) { return self->fb_data; }
